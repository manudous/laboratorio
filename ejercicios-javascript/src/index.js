...
// Crea un parseador que reciba un string en formato CSV y devuelva una colección de objetos. Utiliza destructuring,
...
const fromCSV = (csn) => {
  const [header, ...restRows] =
    csn?.split(/[\n]+/g).map((row) => row.split(",")) ?? [];

  return restRows?.map((row) =>
    header.reduce((acc, prop, idx) => {
      acc[prop] = row[idx];
      return acc;
    }, {})
  );
...
//  Implementar una funcion replaceAt que tome como primer argumento un array, como segundo argumento un índice y
// como tercer argumento un valor y reemplace el elemento dentro del array en el índice indicado. El array de entrada
// no debe de ser mutado, eso es, que debes crear un nuevo array sin modificar el existente.
...
const winnerByYear = (arr, year) => {
  if (arr.find((user) => user.year === year)) {
    const [{ name: name1 }, { name: name2 }, { name: name3 }] = arr
...
// Crear funcion para normalizar una colección de objetos a un objeto, de tal manera que devuelva
// un objeto que tenga como claves las ids de los objetos y como valores los objetos en sí pero sin la id.
...
const result2 = normalize(collection);
console.log(result2);
...
// Ejercicio 5
// Implementa una función para eliminar valores falsys de una estructura de datos. Si el argumento es un objeto,
// deberá eliminar sus propiedades falsys. Si el argumento es un array, deberá eliminar los elementos falsys.
...
const handleArrayCase = (arg) => arg.filter((item) => Boolean(item));

const handleObjectCase = (arg) => {
  let newObj = {};
...
const compact = (arg) => {
  ...
  if (Array.isArray(arg)) {
    return handleArrayCase(arg);
  }
  if (Object.keys(arg).length !== 0) {
    return handleObjectCase(arg);
  } else {
    return arg;
  }
};
